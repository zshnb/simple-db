...
public class HeapFileEncoder {
	 ...
	 * @param outFile    The output file to write data to
	 * @param pageSize The number of bytes per page in the output file
	 ...
	 * @see HeapFile
	 */
	public static void convert(File inFile, File outFile, int pageSize,
		...
		int nrecords = (pageSize * 8) / (nrecbytes * 8 + 1);  //floor comes for free
		...
		ByteArrayOutputStream pageBAOS = new ByteArrayOutputStream(pageSize);
		...
		while (!done) {
			...
			if (recordcount >= nrecords
				...
				for (i = 0; i < (pageSize - (recordcount * nrecbytes + nheaderbytes)); i++)
				...
				pageBAOS = new ByteArrayOutputStream(pageSize);
