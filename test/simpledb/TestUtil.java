...
public class TestUtil {
	 ...
	 */
	public static TupleIterator createTupleList(int width, int[] tupleData) {
		int i = 0;
		List<Tuple> tuples = new ArrayList<>();
		while (i < tupleData.length) {
			Tuple tup = new Tuple(Utility.getTupleDesc(width));
			for (int j = 0; j < width; ++j)
				tup.setField(j, getField(tupleData[i++]));
			tuples.add(tup);
		}

		TupleIterator result = new TupleIterator(Utility.getTupleDesc(width), tuples);
		result.open();
		return result;
	}
