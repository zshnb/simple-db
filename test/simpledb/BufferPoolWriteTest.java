...
public class BufferPoolWriteTest extends TestUtil.CreateHeapFile {
	...
	static class HeapFileDuplicates extends HeapFile {
		...
		public List<Page> insertTuple(TransactionId tid, Tuple t)
			...
			for (int i = 0; i < duplicates; i++) {
				...
				t.setRecordId(new RecordId(t.getRecordId().getPageId(), i));
				p.insertTuple(t);
				writePage(p);
				setPageCount(numPages() + 1);
				dirtypages.add(p);
			}
			return dirtypages;
		}
	...
	public void handleManyDirtyPages() throws Exception {
		...
		Database.getBufferPool().insertTuple(tid, hfd.getId(), Utility.getHeapTuple(1, 2, hfd.getId(), 0));
